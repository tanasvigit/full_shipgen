# driver detail tab: orders

| Property | Value |
|----------|-------|
| **Parent screen** | [driver detail complete](../MASTER__driver-detail-complete.md) |
| **Route** | `fleet-ops.management.drivers.index.details.orders` |
| **Component** | `Driver::Orders` |
...
**Injected services:** `store`, `modalsManager`, `notifications`

**Tasks:** `loadDevices` — use `.perform()`, UI bound via `.isRunning`

- **Setup/teardown:** @action addDevice
- **Setup/teardown:** @action removeDevice
...
`packages/fleetops/addon/components/device/manager.js`
