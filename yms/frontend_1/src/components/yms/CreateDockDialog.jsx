...
export const CreateDockDialog = ({ open, onOpenChange, onCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const created = await docksApi.createDock({
      ...
      toast.success(`${created.name} created (${created.code})`);
      reset();
      onOpenChange(false);
      await onCreated?.();
    } catch (err) {
      toast.error(err.message || "Failed to create dock");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      <DialogContent data-testid="create-dock-dialog" className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display text-lg flex items-center gap-2">
            <Warehouse className="w-5 h-5" /> Create Dock
          </DialogTitle>
          <DialogDescription className="text-[12px] text-slate-500">
            Register dock infrastructure. Code auto-generated (e.g. DK-001).
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <Field label="Dock Name" required>
            <input
              data-testid="dock-name-input"
              className={inputCls}
              value={form.dock_name}
              onChange={(e) => set("dock_name", e.target.value)}
              placeholder="Dock 1"
            />
          </Field>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Dock Type" required>
              <select
                className={inputCls}
                value={form.dock_type}
                onChange={(e) => set("dock_type", e.target.value)}
              >
                {DOCK_TYPES.map((t) => (
          ...
          <DockSupportedTypesField
            ...
            testIdPrefix="dock-vehicle-type"
          />
          ...
          <DockSupportedTypesField
            ...
            testIdPrefix="dock-material-type"
          />
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Status" required>
              <select className={inputCls} value={form.status} onChange={(e) => set("status", e.target.value)}>
                {DOCK_STATUSES.filter((s) => s !== "OCCUPIED").map((s) => (
          ...
          <DialogFooter className="gap-2 sm:gap-2 pt-2">
            ...
            <button type="submit" data-testid="create-dock-submit" disabled={submitting} className="px-4 py-2 text-xs font-semibold bg-slate-900 text-white rounded-md disabled:opacity-50 inline-flex items-center gap-2">
...
export default CreateDockDialog;
