...
export default function DetailDrawerHeader({
  ...
  return (
    <div className="px-4 py-4 pr-12 border-b border-black/[0.06]">
      <div className="min-w-0 space-y-2">
        {overline && (
          <div className="text-[10px] font-mono uppercase tracking-[0.22em] text-[#4B5563]">
            {overline}
          </div>
        )}
        <h2 className="font-display text-xl font-bold tracking-tight text-[#0A0E1A] break-words">{title}</h2>
        ...
        {lastUpdated && (
          <p className="text-[11px] text-[#4B5563] font-mono">Updated {lastUpdated}</p>
        )}
      </div>

      <div
        className="mt-3 pt-3 border-t border-black/[0.05] flex flex-wrap items-center gap-2"
