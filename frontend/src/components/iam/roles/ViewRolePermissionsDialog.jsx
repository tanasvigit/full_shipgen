...
export default function ViewRolePermissionsDialog({ open, onOpenChange, role }) {
  const permissions = role?.permissions || [];

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl" data-testid="view-role-permissions-dialog">
        <DialogHeader>
          <DialogTitle>Role permissions — {role?.name}</DialogTitle>
          <DialogDescription>
            {role?.type || "Role"} · {role?.service || "all services"} · read-only (system / FLB managed role)
          </DialogDescription>
        </DialogHeader>
        <div className="border border-black/[0.08] rounded-md overflow-hidden max-h-72 overflow-y-auto">
          ...
          {permissions.length === 0 ? (
            <div className="px-3 py-8 text-sm text-[#4B5563] text-center">No permissions on this role.</div>
          ) : (
            permissions.map((p) => (
              <div
                key={p?.id || p?.name}
