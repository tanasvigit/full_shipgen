...
export const mapUser = (user) => {
  ...
  return {
    ...
    twoFa: Boolean(user?.two_fa?.enabled ?? user?.twoFaSettings?.enabled ?? user?.two_factor_enabled ?? user?.twoFa?.enabled ?? user?.twoFa),
    twoFaMethod: user?.two_fa?.method || user?.twoFaSettings?.method || user?.twoFa?.method || "email",
    type: user?.type || "user",
