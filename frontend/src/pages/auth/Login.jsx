...
export default function Login() {
    ...
    async function submit(e) {
        ...
        setLoading(true);
        try {
            const outcome = await loginUnified({ email, password, remember });

            if (outcome.sessionType === "yard") {
                toast.success("Signed in to Yard");
                navigate(
                    resolvePostLoginPath(
                        outcome.me?.permissions,
                        redirectTo.startsWith("/yard") ? redirectTo : yardPath("/"),
                    ),
                    { replace: true },
                );
                return;
    ...
    return (
        <div className="space-y-7" data-testid="login-page">
            ...
            <div className="relative border border-dashed border-black/[0.1] bg-[#F5F6F8] rounded-lg p-4 text-xs space-y-1.5 overflow-hidden">
                ...
                <div className="font-mono text-[12px] text-[#4B5563] leading-relaxed">
                    Shipgen admins also receive Yard administrator access automatically. Yard operators sign in with the same form using their assigned credentials.
                </div>
            </div>
