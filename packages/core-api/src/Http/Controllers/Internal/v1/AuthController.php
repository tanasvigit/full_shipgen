...
use Fleetbase\Http\Requests\GoogleLoginRequest;
...
use Fleetbase\Support\GoogleIdTokenVerifier;
...
class AuthController extends Controller
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        if (TwoFactorAuth::isEnabled($user)) {
            $twoFaSession = TwoFactorAuth::startForUser($user);
    ...
    /**
     * Authenticate a console user with a Google Sign-In ID token.
     *
     * @return \Illuminate\Http\Response
     */
    public function loginWithGoogle(GoogleLoginRequest $request)
    {
        if (!GoogleIdTokenVerifier::clientId()) {
            return response()->error('Google Sign-In is not configured on this server.', 503, ['code' => 'google_auth_disabled']);
        }

        $idToken  = $request->input('idToken');
        $clientId = $request->input('clientId') ?: GoogleIdTokenVerifier::clientId();
        $payload  = GoogleIdTokenVerifier::verify($idToken, $clientId);

        if (!$payload) {
            return response()->error('Google Sign-In could not be verified.', 401, ['code' => 'invalid_google_token']);
        }
        ...
        $user = User::whereNull('deleted_at')
            ->where(function ($query) use ($email, $googleUserId) {
        ...
        if (!$user) {
            return response()->error(
                'No Fleetbase account exists for this Google account. Contact your administrator.',
                404,
                ['code' => 'account_not_found']
            );
        }

        if ($user->isType(['customer', 'driver'])) {
            return response()->error('This account cannot access the console.', 403, ['code' => 'invalid_account_type']);
        }

        $hasOrganization = CompanyUser::where('user_uuid', $user->uuid)
            ->whereNull('deleted_at')
            ->exists();

        if ($user->isNotAdmin() && !$hasOrganization) {
            return response()->error('No organization access for this account.', 403, ['code' => 'no_organization']);
        }
