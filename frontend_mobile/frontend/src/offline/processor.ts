...
async function executeItem(item: QueueItem) {
  ...
  switch (item.type) {
    ...
    case "tracking.upload":
      await trackingService.uploadDriverLocation(
        String(item.payload.driverId || ""),
    ...
    for (const item of pending) {
      ...
      try {
        ...
        if (!isRetryableError(error)) {
          const conflict = await resolveConflictFromError({
          ...
          if (conflict) {
            await offlineQueue.remove(item.id);
            logEvent("queue.discarded", { itemId: item.id, type: item.type, kind: conflict.kind });
          } else {
...
/** Requeue dead-letter items and flush — clears stale workflow actions after server state moved on. */
export async function recoverSyncFailures(companyUuid: string) {
  await offlineQueue.requeueDeadLetters(companyUuid);
  return flushOfflineQueue(companyUuid);
}
