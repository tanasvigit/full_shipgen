...
export class OfflineQueue {
  ...
  async requeueDeadLetters(companyUuid?: string) {
    await this.ensureLoaded();
    let changed = false;
    for (const item of this.items) {
      if (item.state === "dead-letter" && (!companyUuid || item.companyUuid === companyUuid)) {
        item.state = "pending";
        item.retries = 0;
        changed = true;
      }
    ...
    return this.getDeadLetter(companyUuid);
  }

  async clearDeadLetters(companyUuid?: string) {
    await this.ensureLoaded();
