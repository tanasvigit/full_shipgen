...
export const workflowService = {
  ...
  async start(orderId: string) {
    ...
    try {
      await apiRequest("/orders/start", {
        method: "PATCH",
        body: { order: orderId },
      });
      logWorkflow("start.applied", { orderId, via: "orders/start" });
      return;
    } catch (error) {
      const status = (error as { status?: number })?.status;
      const message = error instanceof Error ? error.message : "";
      if (status === 400 && message.toLowerCase().includes("already been started")) {
        throw error;
      }
      if (status === 401 || status === 403) {
        captureError(error, { operation: "workflow.start.orders_start", orderId });
        throw new Error(message || "Not authorized to start this trip.");
      }
      if (status !== 400) {
        captureError(error, { operation: "workflow.start.orders_start", orderId });
      }
