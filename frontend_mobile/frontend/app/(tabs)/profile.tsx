...
export default function Profile() {
  ...
  useEffect(() => {
    if (!driverTrackId) return;
    void driverService
      .getById(driverTrackId)
      .then((driver: any) => {
        if (typeof driver?.online === "boolean") {
          setOnline(driver.online);
        } else if (driver?.status) {
          setOnline(String(driver.status).toLowerCase() === "online");
        }
      })
      .catch(() => undefined);
  }, [driverTrackId]);
  ...
  const toggleOnline = useCallback(async () => {
    if (!driverTrackId) {
      Alert.alert("Driver profile required", "Link this user to a driver record to go online.");
      return;
    }
    const next = !online;
    setOnline(next);
    try {
      await driverService.toggleOnline(driverTrackId, next);
    } catch (error) {
      setOnline(!next);
      Alert.alert("Unable to update status", error instanceof Error ? error.message : "Try again.");
    }
  }, [driverTrackId, online]);
